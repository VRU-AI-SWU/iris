---
type: concept
---

<!-- lang-switch -->
**English** · [ภาษาไทย](esco-ontology.th.md)

## Definition
ESCO (European Skills, Competences, Qualifications and Occupations) is a multilingual classification system developed by the European Commission. It contains ~13,890 skills/competences and ~3,008 occupations with relationships between them, available in 27 languages. It is the most widely used skill taxonomy in the NLP/labour market literature.

## Papers That Discuss This
*(populated via Obsidian backlinks)*

## Related Concepts
[[onet-taxonomy]] · [[skill-ontology]] · [[skill-normalisation]]

## Relevance to Iris
ESCO is the dominant taxonomy in the skill extraction literature but has limited Thai language support and was designed for the European labour market context. Mapping our emergent Thai skill vocabulary to ESCO post-hoc for cross-context comparability is feasible but requires validation that ESCO covers Thai CS programme skills adequately. ESCO's multilingual nature (27 languages) does not include Thai.
