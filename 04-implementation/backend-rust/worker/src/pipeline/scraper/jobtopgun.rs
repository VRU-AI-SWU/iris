/// Scraper for jobtopgun.com
///
/// Search URL: https://www.jobtopgun.com/search?kwd={query}&page={n}
...
const BASE: &str = "https://www.jobtopgun.com";
...
pub async fn scrape(
    ...
    for page in 1..=MAX_PAGES {
        let url = format!("{BASE}/search?kwd={query}&page={page}");
        let html = fetch(http, &url).await?;

        let page_postings: Vec<ScrapedPosting> = {
            use scraper::{Html, Selector};
            let doc = Html::parse_document(&html);
            let card_sel = Selector::parse("div.job-box, div.job-card, li.job-row").unwrap();
            ...
            card_htmls
                ...
                .filter_map(|(card_html, link)| {
                    ...
                    let description = super::common::select_text(
                        &card_doc,
                        ".job-desc, .description, .job-summary",
                    );
                    Some(ScrapedPosting {
                        id: posting_id("jobtopgun", &link),
                        source: "jobtopgun".to_string(),
                        title,
                        company,
                        description,
                        requirements: None,
    ...
    tracing::info!(source = "jobtopgun", career_path, count = postings.len(), "scrape done");
    Ok(postings)
}
