/// Scraper for th.jobsdb.com (SEEK Thailand).
///
/// Search URL: https://th.jobsdb.com/th/search-jobs/{query}/{page}
...
const BASE: &str = "https://th.jobsdb.com";
...
pub async fn scrape(
    ...
    for page in 1..=MAX_PAGES {
        let url = format!("{BASE}/th/search-jobs/{encoded}/{page}");
        ...
        let page_postings: Vec<ScrapedPosting> = {
            ...
            let card_sel = Selector::parse(
                "article[data-automation='normalJob'], div[data-testid='job-card']",
            )
            .unwrap();
            ...
            card_htmls
                ...
                .filter_map(|(card_html, link)| {
                    ...
                    let title = super::common::select_text(
                        &card_doc,
                        "h1[data-automation='job-title'], a[data-automation='job-title']",
                    )?;
                    let company = super::common::select_text(
                        &card_doc,
                        "span[data-automation='job-company-name']",
                    );
                    let description = super::common::select_all_text(
                        &card_doc,
                        "div[data-automation='jobAdDetails'], .job-description",
                    );
                    Some(ScrapedPosting {
                        id: posting_id("jobsdb", &link),
    ...
    tracing::info!(source = "jobsdb", career_path, count = postings.len(), "scrape done");
    Ok(postings)
}
