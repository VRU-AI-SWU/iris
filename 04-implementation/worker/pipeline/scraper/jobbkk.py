"""JOBBKK.com scraper (placeholder — selectors to be validated against live site)."""
import hashlib
import logging
...
class JobbkkScraper(BaseScraper):
    source_name = "jobbkk"
    ...
    def _get_search_urls(self, career_path: str) -> list[str]:
        ...
        return [f"{self.base_url}/jobs/search?keyword={kw.replace(' ', '+')}" for kw in keywords[:2]]

    def _scrape_page(self, url: str, career_path: str) -> list[dict]:
        soup = self._get(url)
        if not soup:
            return []
        cards = soup.select(".job-item, .job-list, [class*='job']")
        postings = []
        for card in cards:
            ...
            posting_id = hashlib.md5(title.encode()).hexdigest()[:12]
            postings.append({
